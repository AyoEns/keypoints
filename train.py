...
from utils.keypoinst_dataloader import CenternetDatasets
...
if __name__ == "__main__":
    ...
    # model_path = 'model_data/hrnetv2_w18_imagenet_pretrained.h5'
    model_path = ''
    #---------------------------------------------------------------------#
    ...
    if ngpus_per_node > 1:
        with strategy.scope():
            model = centernet([input_shape[0], input_shape[1], 3], num_classes=num_classes, backbone=backbone, mode='train', max_objects=700)
            if model_path != '':
                #------------------------------------------------------#
                #   载入预训练权重
                #------------------------------------------------------#
    ...
    if True:
        ...
        train_dataloader    = CenternetDatasets(train_lines, input_shape, batch_size, num_classes, train = True, max_objects=3000)
        val_dataloader      = CenternetDatasets(val_lines, input_shape, batch_size, num_classes, train = False, max_objects=3000)
