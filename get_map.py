...
if __name__ == "__main__":
    ...
    if map_mode == 0 or map_mode == 1:
        print("Load model.")
        centernet = CenterNet(confidence = 0.01, nms_iou = 0.1)
        print("Load model done.")

        print("Get predict result.")
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(VOCdevkit_path, "VOC2007/JPEGImage/"+image_id+".jpg")
            ...
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                ...
                for obj in root.findall('object'):
                    ...
                    if obj_name not in class_names:
                        continue
                    Keypoints_XY  = obj.find('Keypoints_XY')
                    x = Keypoints_XY.find("keypoints_x").text
                    y = Keypoints_XY.find("keypoints_y").text
                    left    = float(float(x) - 5)
                    top     = float(float(y) -5)
                    right   = float(float(x) + 5)
                    bottom  = float(float(y) + 5)
                    if difficult_flag:
                        new_f.write("%s %s %s %s %s difficult\n" % (obj_name, left, top, right, bottom))
                    else:
                        new_f.write("%s %f %f %f %f\n" % (obj_name, left, top, right, bottom))
